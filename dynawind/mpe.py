...
Overview
-------------------
The modal parameter package in dynawind handles all functions that are related with the modal parameter estimation.
...
def wj(f, fmin, fmax, MO=0, conv="normal", base="exp"):
    """ Calculates the base polynomials
    
    :param f: frequency value to convert
    :param fmin: min. of the considered frequency range
    :param fmax: max. of the considered frequency range
    :param MO: Model order
    :parm conv: either 'normal' : converts the frequency to the base polynomial, or 'inverse' : returns the frequency from a base polynomial, typically used to convert poles back to the regular frequency domain
    :returns: the base polynomial value
    """
    if base == "exp":
...
def correlogram(Signals):
    """ Calculates the correlogram from the signals
...
def LSCF(
    ...
    """ LSCF based modal parameter estimation algorithm
    
    :param Signals: List of :class:`dynawind.dynawind.Signal` to be processed into the modal parameter
    :param modelorder: considered maximum modelorder
    :type modelorder: int.
    :param min_modelorder: minimum model order to consider
    :type min_modelorder: int.
    :param base: polynomial base to use in :func:`dynawind.mpe.wj`
    :param fband: frequency band
    :param windowlenght: window length
    :param ref: Reference output
    :param beta: Beta to be used in the correlogram approach as window
    :param powerspectrum: approach to calculate the powerspectrum, default : correlogram
    :param plotresult: Plot the results of the curve fit
    :type plotresult: bool
    ...
    for i in range(n_o):
        Gxx[G.shape[0] * i : G.shape[0] * (i + 1), :] = np.transpose(
            np.matlib.repmat(G[:, i], modelorder, 1)
        )
    ...
    for i in range(modelorder):
        Omega_all[:, i * n_o : (i + 1) * n_o] = np.transpose(
            np.kron(np.eye(n_o), Omega[:, i])
        )
    Omega = Omega_all
    ...
    for MO in range(min_modelorder, modelorder):
        ...
        poles[: int(MO - 1), int(MO - 1)] = (
            temp_poles[::1] + beta
        )  # Compensate for the window
...
def SSICOV(Signals, modelorder=48, jb=10):
    """ SSICOV based operational modal analysis 
    :param Signals: 
    ...
    for MO in range(2, modelorder, 2):
        O_to = O[: No * (jb - 1), :MO]
        O_bo = O[No:, :MO]
        A, res, rank, s = np.linalg.lstsq(O_to, O_bo, rcond=-1)
...
def DBSCAN(poles, modes, plotClusters=False, min_clustersize=5, plot2stabchart=False, ax=None):
    ...
    X = np.transpose(
        np.array(
            (
                np.reshape(abs(poles) / 2 / np.pi, -1) * 10,
                np.reshape(np.divide(-np.real(poles), abs(poles)) * 10, -1),
            )
        )
    )
    ...
    MO = np.matlib.repmat(np.linspace(0,poles.shape[0]-1,poles.shape[0]),poles.shape[0],1)
    MO = np.reshape(MO, -1)*2+2
    X = X[~Xtest, :]
    ...
    cluster_size = np.zeros(n_clusters_)
    ...
    for k, col in zip(sorted(unique_labels), colors):
        ...
        mo = MO[class_member_mask & core_samples_mask]
        # %% Plot results of clustering
        if plotClusters or plot2stabchart:
            if ax is None:
                ax = plt.gca()
        ...
        if xy.size > 0:
            ...
            cluster_size[k] = xy.shape[0]
            if cluster_size[k] < min_clustersize:
                freq_median[k] = np.nan
    ...
    damp_median=damp_median[~np.isnan(freq_median)]
    damp_std=damp_std[~np.isnan(freq_median)]
    cluster_size=cluster_size[~np.isnan(freq_median)]
    freq_mean = freq_mean[~np.isnan(freq_median)]
    freq_median = freq_median[~np.isnan(freq_median)]

    return freq_mean, freq_median, freq_std, damp_median, damp_std, modes_mean, cluster_size
...
def tracking(mpe, trackinglist, root="."):
    ...
    for track in trackinglist:
        ...
        TrackIndex = freqtrack(
            mpe.freq_median, target
        )  # Current tracking only based on frequency
...
# %%
class MPE(object):
    """ Main class for modal parameter estimation """
    def __init__(
        self,
        signals,
        algorithm=LSCF,
        ...
        self.direction = (
            direction
        )  # This input allows to differentiate between e.g. the tracking set for FA and SS
        ...
        if not settings:
            (settings, clusterSettings, trackinglist) = self.mpe_config(
        ...
        # Actual MPE
        self.all_poles, self.all_modes = self.algorithm(
            signals, modelorder=self.modelorder, **settings
        )
        self.freq_mean, self.freq_median, self.freq_std, self.damp_median, self.damp_std, self.modes_mean, self.cluster_size = self.cluster_algorithm(
            self.all_poles, self.all_modes, **cluster_settings
        )
        self.sortResults()
    ...
    def mpe_config(self, direction=None):
        ...
        if self.source != "unknown":
            ...
            resource_path = "/".join(("config", site.lower(), location + "_mpe.ini"))
                ...
                for section in config.sections():
                    if section == "Modal analysis":
                        for key in config["Modal analysis"]:
                            if key == "modelorder":
                                self.modelorder = int(
                                    config["Modal analysis"]["modelorder"]
                                )
                            elif key == "algorithm":
                                if (
                                    config["Modal analysis"]["algorithm"].lower()
                                    == "plscf"
                                ):
                                    self.algorithm = LSCF
                                elif (
                        ...
                        if direction:
                            if config[section]["direction"] == direction:
                                trackinglist.append(
                                    TrackTarget(section, freq, config[section])
                                )
                        else:
                            trackinglist.append(
                                TrackTarget(section, freq, config[section])
    ...
    def stab_chart(self, signal=None, xlim=[0, 2], include_clusters=False, select_clusters=False, ax =None):
        """ Plot the stabchart of the mpe results. Will draw all poles as crosses, with dotted lines to highlight the result of the clustering.
        
        :param signal: signal that is used for the backdrop of the stabchart
        :type signal: :class:`dynawind.dynawind.signal`
        :param xlim: Lower and upper frequency limit of the spectrum in the background
        :type xlim: list
        :param include_clusters: Plot stabilization chart with the clusters highlighted
        :type include_clusters: Bool.
        :param select_clusters: Manually select clusters from the stabchart, the selected clusters will be added to the tracking list of the considered mpe object
        :type select_clusters: Bool.
        """
        ...
        if not include_clusters:
            ax1.plot(
                np.transpose(test),
                np.linspace(1, np.size(self.all_poles, 0), num=np.size(self.all_poles, 0))
                * 2,
                "rx",
            )
        else:
            ...
            self.plotClusters(plot2stabchart=True, ax= ax1)
        ax1.vlines(self.freq_median, 0, np.size(self.all_poles, 0) * 2, "k", ":")

        ax2 = ax1.twinx()
        signal.plotPSD(xlim=xlim, ax = ax2)
        if select_clusters:
            selected_clusters = plt.ginput(n=-1,show_clicks=True)
            trackinglist = [TrackTarget('Mode',x[0],{'track_mode':'frequency','direction':None}) for x in selected_clusters]
            self.tracked = tracking(self,trackinglist)
        return ax1, ax2
    def sortResults(self):
        ...
        self.cluster_size = self.cluster_size[p]

    def stability_index(self):
        """ The stability index is a metric to quantify the accuracy of a pole, it takes into account the number of poles and the std of damping and frequency."""
        pole_2_modelorder_ratio = (self.modelorder-self.cluster_size)/self.modelorder
        cluster_area = np.multiply(self.freq_std,np.divide(self.damp_std,self.damp_median))
        self.stability_index=np.divide(1-pole_2_modelorder_ratio,cluster_area)/5000
    def plotModeshapes(self):
        ...
        realmodes = np.divide(
            realmodes, np.matlib.repmat(modesmax, realmodes.shape[0], 1)
        )
        ...
        for i in range(realmodes.shape[1]):
            plt.plot(
                np.real(realmodes[:, i]),
                self.levels,
                label="f:{0:.3f}(Hz)|d:{1:.3f}(%)".format(
                    self.freq_median[i], self.damp_median[i]
                ),
            )
        # Add legend
        plt.legend()
    ...
    def plotClusters(self, plot2stabchart=False, ax = None):
        if plot2stabchart:
            self.cluster_algorithm(self.all_poles, self.all_modes, plotClusters=False,plot2stabchart=True, ax=ax)
        else:
            self.cluster_algorithm(self.all_poles, self.all_modes, plotClusters=True, ax=ax)
                ...
                export.update(
                    {
                        ...
                        + "_"
                        + track["name"]
                        + "_FREQ": track["freq_median"],
                        "std_"
                        + self.source
                        + "_"
                        + track["name"]
                        + "_FREQ": track["freq_std"],
                        "median_"
                        + self.source
                        + "_"
                        + track["name"]
                        + "_DAMP": track["damp_median"],
                        "std_"
                        + self.source
                        + "_"
                        + track["name"]
                        + "_DAMP": track["damp_std"],
                    }
                )
        else:
            if self.direction:
    ...
    def __repr__(self):
        ...
        print("Freq.\t| Std.\t|| Damp\t| Std. \t| Stability index")
        print("(Hz).\t| (Hz).\t|| (%).\t| (%).\t|")
        print("---------------------------------------------------")
        for i in range(len(self.freq_median)):
            print(
                "{0:.3f}\t|{1:.3f}\t||{2:.2f}\t|{3:.3f}".format(
                    self.freq_mean[i],
                    self.freq_std[i],
                    self.damp_median[i],
                    self.damp_std[i],
                )+'\t|'+stabindex2str(self.stability_index[i])
                
            )
        return ""
def stabindex2str(stab_index):
    if stab_index>1:
        stab_index=1
    return int(stab_index*10)*'*'
